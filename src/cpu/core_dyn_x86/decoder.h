...
static void dyn_read_byte(DynReg * addr,DynReg * dst,bool high,bool release=false) {
	...
	cache_addb(0x0c);
	cache_addw(0x048b); // mov eax,paging_tlb_read[eax*TYPE Bit32u]
	cache_addb(0x85);
	cache_addd((Bit32u)(paging_tlb_read.data()));
...
static void dyn_write_byte(DynReg * addr,DynReg * val,bool high,bool release=false) {
	...
	cache_addb(0x0c);
	cache_addw(0x0c8b); // mov ecx,paging_tlb_write[ecx*TYPE Bit32u]
	cache_addb(0x8d);
	cache_addd((Bit32u)(paging_tlb_write.data()));
...
static void dyn_read_word(DynReg * addr,DynReg * dst,bool dword,bool release=false) {
	...
	// mov tmp, [8*tmp+paging_tlb_read(rbp)]
	opcode(tmp)
	        .set64()
	        .setea(5, tmp, 3, (Bits)paging_tlb_read.data() - (Bits)&cpu_regs)
	        .Emit8(0x8B);
...
static void dyn_write_word(DynReg * addr,DynReg * val,bool dword,bool release=false) {
	...
	opcode(5).setrm(tmp).setimm(12,1).Emit8(0xC1); // shr tmpd,12
	// mov tmp, [8*tmp+paging_tlb_write(rbp)]
	opcode(tmp)
	        .set64()
	        .setea(5, tmp, 3, (Bits)paging_tlb_write.data() - (Bits)&cpu_regs)
	        .Emit8(0x8B);
	opcode(tmp).set64().setrm(tmp).Emit8(0x85); // test tmp,tmp
	const Bit8u *nomap=gen_create_branch(BR_Z);
	//mov [tmp+src], dst
